...
describe('should display text or logo', () => {
  it('should display default title', () => {
    const tree = renderer.create(<Header t={text => text} />).toJSON();
    expect(tree).toMatchSnapshot();
  });
  it('should display title', () => {
    const tree = renderer.create(<Header title="Foo Title" t={text => text} />).toJSON();
    expect(tree).toMatchSnapshot();
  });
...
it('should display heading', () => {
  const tree = renderer
    .create(<Header t={text => text} heading="<h1>Heading text</h1>" />)
    .toJSON();
  expect(tree).toMatchSnapshot();
});

it('should display menu when user is authenticated', () => {
  const tree = renderer.create(<Header t={text => text} authenticated />).toJSON();
  expect(tree).toMatchSnapshot();
});
